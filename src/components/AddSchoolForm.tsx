...
const AddSchoolForm = forwardRef<HTMLDivElement, AddSchoolFormProps>(({ onSchoolCreated }, ref) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        console.warn('Backend API call fallback to client state update:', err);
      }

      // Format coach names
      const hcName = [formData.headCoachFirstName.trim(), formData.headCoachLastName.trim()]
        .filter(Boolean)
        .join(' ') || 'Head Coach';
      ...
      const newSchool: School = {
        ...
        coachesDisplay: scEmail ? `${hcName} (HC), Strength Coach (SC)` : `${hcName} (HC)`,
      };
  ...
  return (
    <div ref={ref} className="bg-[#1C2128] border border-[#2D333B] rounded-xl p-6 shadow-xl mb-8">
      <div className="flex items-center space-x-2 mb-6">
        <h2 className="text-lg font-bold text-white tracking-wide">Add New School</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-end pt-2">
          <button
            type="submit"
            disabled={loading}
            className="flex items-center space-x-2 bg-[#FAE035] hover:bg-[#E5CD25] text-black font-extrabold text-xs px-6 py-3 rounded-lg shadow-md hover:shadow-lg transition duration-200 cursor-pointer disabled:opacity-50"
          >
            <Send size={14} className="rotate-45" />
            <span>{loading ? 'Creating School...' : 'Create School & Send Invites'}</span>
          </button>
        </div>
      </form>
    </div>
